...
"
    ...
    foreach ($worksheet in $workbook.Worksheets) {
		# Array que almacena los declares
		$goDeclares = @()
		...
		for ($i = 1; $i -le $lastRow; $i++) {
            ...
            if ($contenidoCelda.Length -gt 70 -or $contenidoCelda -like '*DECLARE*') {
				...
				if($contenidoCelda -like '*DECLARE*'){
					$goDeclares += $contenidoCelda
				}
				...
				if ($contadorLineasTotales % 45 -eq 0) {
					...
					foreach ($declare in $goDeclares) {
						$contenidoColumna += $declare + "`r`n"
					}					
				}
			}
		}
	...
	$archivoSalida = "\script generado\20240718-XXXX-003-DAT-$($nombreArchivoSinExtension).sql"
	$contador++
